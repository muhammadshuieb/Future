...
export function QoeOverviewPage() {
  const { t, isRtl } = useI18n();
  const { user } = useAuth();
  const [data, setData] = useState<unknown>(null);
  const ok = canSeeEnterprise(user, "view_qoe");
  useEffect(() => {
    if (!ok) return;
    void (async () => {
      const r = await apiFetch("/api/qoe/overview");
  ...
  return (
    <div className="p-4" dir={isRtl ? "rtl" : "ltr"}>
      <Card className="p-4">
        <h1 className="mb-2 text-lg font-bold">{t("nav.qoeOverview")}</h1>
...
export function RadiusMonitorOverviewPage() {
  const { t, isRtl } = useI18n();
  const { user } = useAuth();
  const [data, setData] = useState<unknown>(null);
  const ok = canSeeEnterprise(user, "view_radius_monitor");
  useEffect(() => {
    if (!ok) return;
    void (async () => {
      const r = await apiFetch("/api/radius-monitor/overview");
      if (r.ok) setData(await r.json());
    })();
  }, [ok]);
  if (!ok) return <div dir={isRtl ? "rtl" : "ltr"}>{t("api.error_403")}</div>;
  return (
    <div className="p-4" dir={isRtl ? "rtl" : "ltr"}>
      <Card className="p-4">
        <h1 className="mb-2 text-lg font-bold">{t("nav.radiusMonitor")}</h1>
        <pre className="text-xs">{data ? JSON.stringify(data, null, 2) : t("common.loading")}</pre>
      </Card>
    </div>
  );
