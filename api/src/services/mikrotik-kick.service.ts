...
export async function mikrotikKickUsername(opts: {
  ...
  if (config.dmaMode) {
    ...
    if (!(await hasTable(pool, "nas"))) {
      return { ok: false, message: "nas_missing" };
    }
    const ncols = await getTableColumns(pool, "nas");
    if (!ncols.has("apiusername") || !ncols.has("apipassword")) {
      return { ok: false, message: "mikrotik_api_not_configured" };
    }
    const [nrows] = await pool.query<RowDataPacket[]>(
      `SELECT nasname, apiusername, apipassword FROM nas WHERE nasname = ? LIMIT 1`,
      [nasIp]
    );
    const nrow = nrows[0];
    if (!nrow) return { ok: false, message: "nas_row_not_found" };
    const apiUser = String(nrow.apiusername ?? "").trim();
    const apiPass = String(nrow.apipassword ?? "").trim();
    if (!apiUser || !apiPass) {
