...
const MAX_BYTES = Math.min(
  Math.max(1, parseInt(process.env.RESTORE_MAX_SQL_BYTES ?? `${200 * 1024 * 1024}`, 10)),
  500 * 1024 * 1024
);
...
export async function importSqlBufferIntoAppDatabase(
  ...
  try {
    let err: string;
    try {
      err = await pipeFileToMysql(tmpFile);
    } catch (e) {
    ...
    return { ok: true, detail: { bytes: buffer.length, appliedSchemaExtensions } };
  } finally {
