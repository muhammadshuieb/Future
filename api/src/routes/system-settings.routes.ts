...
const bodySchema = z.object({
  ...
  backup_alert_enabled: z.boolean().optional(),
...
router.put("/", routePolicy({ allow: ["admin", "manager"] }), async (req, res) => {
  ...
  try {
    ...
    const next: SystemSettingsInput = {
      ...
      backup_alert_enabled: parsed.data.backup_alert_enabled ?? cur.backup_alert_enabled,
      backup_alert_phone: parsed.data.backup_alert_phone ?? cur.backup_alert_phone,
